...
Дата 2026-09-23. **DESIGN FROZEN.** Ничего не реализовано, production не изменён.
Scope закрыт: новые функции до реализации не добавляются.
...
## 7. Финальные решения (design freeze)

### 7.1 Критерий стабилизации PR #99 — условие старта FX-работ

Стабилизация достигнута, когда выполнено **всё**:

1. проведён controlled production deployment по `PRODUCTION_DEPLOY_RUNBOOK.md`;
2. post-deploy integrity audit успешен;
3. пройдены T/smoke-проверки критических финансовых сценариев;
4. прошёл **минимум один реальный успешный цикл** синхронизации Tochka/Bybit;
5. проверены split и управленческая аналитика **на production**;
6. **24 часа без новых HIGH/CRITICAL инцидентов**.

Оговорки:

* исторические **126 дублей FIN-03 новым инцидентом не считаются**, пока их
  количество не изменилось (детектор в `db_integrity_audit.sql` должен давать
  ровно 126);
* при появлении нового HIGH/CRITICAL отсчёт 24 часов начинается **заново**
  после исправления.

### 7.2 Immutable BEFORE/AFTER вокруг B1

До изменения 33 операций формируется **неизменяемый артефакт BEFORE**, после
B1 — идентичный по структуре **AFTER**. Состав (одинаковый для обоих):

| Блок | Содержание |
|---|---|
| Операции | все 33: id, original amount, currency, occurred_on, текущая рублёвая оценка, курс ЦБ и его дата, proposed/actual `base_amount`, delta |
| Отчётность | ОПиУ, ДДС, Dashboard totals |
| Разрезы | Projects, Employees, Contractors, Categories |
| Остатки | account balances по каждому счёту |
| Инварианты | `Σ splits = transactions.amount`, `Σ base_amount строк = base_amount операции`, счётчики строк |
| Контроль | checksums и counts по затронутым таблицам |

**Требование приёмки B1**: **100 % изменений финансовых показателей должны
объясняться дельтой B1**. Остатки счетов и движение денег **в оригинальных
валютах не меняются вообще** (B1 трогает только `base_amount` и FX-поля).

**STOP-условие**: любая необъяснённая дельта → B2 не запускается, разбор причин.

Артефакты складываются рядом с `fx_backfill_audit` и сохраняются вне БД
(выгрузка), чтобы AFTER сверялся с независимой копией BEFORE.

### 7.3 `FX_RATE_MISSING` — поведение и алерт

* операция **не блокируется** ни при создании, ни при импорте;
* сохраняется `fx_status='missing'`, `base_amount = NULL`;
* **никаких** `1:1`, `0` и latest-rate fallback;
* в интерфейсе и отчётах — явная пометка «**Не пересчитано**»;
* такие строки **не входят молча** в рублёвые итоги: они выводятся отдельной
  строкой с количеством и суммой в оригинальной валюте;
* **один централизованный алерт** `FX_RATE_MISSING count > 0` (не по одной
  операции): показывает количество, список валют, минимальную и максимальную
  дату missing-операций. Встраивается в существующий механизм `src/lib/alerts.ts`;
* после восстановления источника — **контролируемый** resolve через `fx_repin`
  с полным audit trail (`reason='resolve missing FX'`), не автоматически.

### 7.4 Недоступность источника котировок

Недоступность ЦБ и зеркала **не ломает** создание финансовых операций.

`fx_calendar` используется **только там, где для конкретной календарной даты
подтверждена effective-rate semantics** (`derivation='SOURCE_DECLARED'`, либо
явный `CARRY_FORWARD`, проставленный процедурой бэкофилла). Нет подтверждения
для даты — `FX_RATE_MISSING`. Молчаливая интерполяция неизвестной рабочей даты
предыдущим курсом **запрещена** (поведение проверено тестом FX6).

### 7.5 Порядок работ после стабилизации PR #99

```
feature/fx-cbr-cutover-2026-08-20
  → миграции 0093–0099
  → тесты FX1–FX20
  → регрессия T1–T14
  → AUTHZ (21)
  → SPLIT (36)
  → rehearsal на объёме прода
  → отдельный controlled deployment (без backfill)
  → B1 (33 actual операции)
  → STOP + BEFORE/AFTER reconciliation
  → отдельное подтверждение
  → B2 (4 инвойса + 5 обязательств)
```

**FIN-03 cleanup и `opening_balance` в FX-PR не входят** — это отдельный этап.
