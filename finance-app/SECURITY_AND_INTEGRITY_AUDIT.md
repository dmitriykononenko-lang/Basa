...
# 10. Этап 3 — forensic по FIN-03 и архитектура исправлений

Отдельные документы (production не изменялся, ничего не исправлялось и не удалялось):

- **`FIN03_FORENSICS.md`** — forensic-проверка всех 126 случаев: классификация, evidence profile,
  экономический эффект по счетам / кассе / ОПиУ / ДДС / дашборду, root cause, оценка повторяемости.
- **`REMEDIATION_PLAN.md`** — пересмотр severity по реальному финансовому impact, архитектура
  исправления для T1–T9 и FIN-01..03, финальная таблица
  `Finding | Severity | Impact | Can recur | Root cause | Fix | Migration | Regression test`,
  **PRODUCTION REMEDIATION ORDER** с зависимостями, план правки исторических данных и rollback.
- **`scripts/fin03/forensics.sql`** — read-only выгрузка по каждому случаю (все запрошенные поля).
- **`scripts/fin03/dry_run_remediation.sql`** — dry-run `BEFORE → proposed → AFTER` + контрольные
  суммы; **ни одного DML**, реальные DELETE/UPDATE закомментированы.

## 10.1 Итог по FIN-03

| Класс | Кол-во |
|---|---:|
| CONFIRMED_DUPLICATE | **126** |
| PROBABLE_DUPLICATE / LEGITIMATE_PAIR / UNKNOWN | 0 / 0 / 0 |

Экономический эффект (проверено симуляцией на production, только SELECT):
**общий cash не искажён** (8 345 957 до и после); искажены остатки 9 счетов (сумма искажений = 0,
сейчас замаскировано значениями `opening_balance`); **выручка ОПиУ за янв–мар 2025 завышена в
3,2 раза** — 1 751 930,02 ₽ вместо 546 432,93 ₽, прибыль завышена на 1 205 497,09 ₽; в ДДС
завышены приток и отток, сальдо верное. Root cause: CSV-импортёры не заполняют `source`/
`external_id`, дедуп Точки их не видит; трактовка перевода у двух импортёров разная.
**Повторяемо сегодня.**

## 10.2 Пересмотр severity (детали и обоснование — `REMEDIATION_PLAN.md` §1)

| ID | Было | Стало |
|---|---|---|
| T1 / CONC-05 — начисление ЗП при GET-рендере | HIGH | **CRITICAL** |
| T9 / CONC-09 — split и склейка `insert`+`delete` | HIGH | **CRITICAL** |
| FIN-01 — неизвестный курс считается 1:1 молча | MEDIUM | **HIGH** |
| T2 / CONC-01 — инвойс ≠ Σ позиций | HIGH | HIGH |
| T4 / CONC-03 — переплата обязательства | HIGH | HIGH |
| T8 / CONC-07 — lost update | HIGH | HIGH |
| T7 / CONC-06 — TOCTOU тротлинга | MEDIUM | **LOW** (значим только как триггер T6) |

Конкурентность финансового ядра приоритетнее прежних LOW-находок безопасности.
Порядок исполнения — `REMEDIATION_PLAN.md` §5. Шаг «историческая чистка FIN-03» **не** начинать
раньше шага «исправление импорта», иначе дубли вернутся при следующем CSV-импорте.
