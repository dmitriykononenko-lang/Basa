...
Это **единственный** способ, который поддерживает скрипт: он проверяет, что
`PGPASSWORD` задан, и отказывается работать без него. Файл `~/.pgpass` для этого
прогона **не нужен и не используется** — не создавайте его.

⚠️ Никогда не вводите пароль как часть команды (`PGPASSWORD=… bash …`,
`printf '…пароль…'`, `echo пароль > …`) — такая строка сохранится в истории
оболочки (`~/.zsh_history`) в открытом виде.
...
Перед итоговой плашкой обязательно будут три строки сверки:

```
  [restore] сверено показателей: N, расхождений: 0
  [postmig] финансовых расхождений: 0 · структурных изменений: K
  ОТКАТ ЧИСТЫЙ: все N показателей вернулись к состоянию после restore
```

`K > 0` — это нормально: миграции добавляют функции, индексы и ограничения.
А вот любое **финансовое** расхождение после миграций останавливает скрипт
(`ОСТАНОВ: после 0086–0092 изменились финансовые показатели …`), и итоговая
плашка «ВСЁ ПРОШЛО УСПЕШНО» в этом случае не печатается.
...
Если когда-то раньше, по предыдущей версии этой инструкции, вы создавали
`~/.pgpass` с этим проектом — удалите из него строку (команда пароль не содержит):

```bash
[ -f ~/.pgpass ] && grep -v 'postgres.kmvsjozxjosmkhvphzdz' ~/.pgpass > ~/.pgpass.new; [ -f ~/.pgpass.new ] && mv ~/.pgpass.new ~/.pgpass && chmod 600 ~/.pgpass
```

И проверьте, что пароль не попал в историю — вывод должен быть пустым:

```bash
grep -n -E 'PGPASSWORD=[^$ ]|pgpass' ~/.zsh_history ~/.bash_history 2>/dev/null
```
