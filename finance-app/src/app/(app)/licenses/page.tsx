...
type RawPurchase = {
  ...
  vendor_counterparty_id: string | null; note: string | null;
};
type RawDeal = {
...
export default async function LicensesPage() {
  ...
  const [{ data: deals }, { data: counterparties }, { data: projects }, { data: fxRows }] = await Promise.all([
    supabase
      .from("license_deals")
      .select("id, title, sale_amount, currency, sold_on, expected_cost, status, note, client_counterparty_id, project_id, purchases:license_purchases(id, amount, currency, purchased_on, vendor_counterparty_id, note)")
      .eq("team_id", team.id)
